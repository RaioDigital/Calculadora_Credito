...
function Calcular(opcao) {
  ...
  if (!checkInput(document.form1.periodo)) {
    ...
    opcao = 1;
  } else if (!checkInput(document.form1.juros)) {
    if (
    ...
    opcao = 2;
  } else if (!checkInput(document.form1.parcela)) {
    if (
      !checkInput(document.form1.periodo) ||
      !checkInput(document.form1.juros) ||
      !checkInput(document.form1.montante)
    ...
    opcao = 3;
  } else if (!checkInput(document.form1.montante)) {
    if (
      !checkInput(document.form1.juros) ||
      !checkInput(document.form1.parcela) ||
      !checkInput(document.form1.periodo)
  ...
  if (opcao != 2) {
    ...
    if (!isFloat(s)) {
      alert(
        "Taxa de Juros deve ser um valor númerico, tendo a vírgula(,) como delimitador da parte fracionária."
      );
      return;
    }
  ...
  if (opcao != 3) {
    ...
    if (!isFloat(s)) {
      alert(
        "Valor da prestação deve ser um valor númerico, tendo a vírgula(,) como delimitador dos centavos."
      );
      return;
    }
  ...
  if (opcao != 4) {
    ...
    if (!isFloat(s)) {
      alert(
        "Valor do Financiamento deve ser um valor númerico maior que zero, tendo a vírgula(,) como delimitador dos centavos."
      );
      return;
    }
  ...
  if (opcao == 1) {
    per_int =
      Math.log(1 - (montante_float * juros_float) / parcela_float) /
      Math.log(1 / (1 + juros_float));
  ...
  if (opcao == 2) {
    ...
    while (true) {
      ...
      suposto_parcela =
        (montante_float * suposto_juros) /
        (1 - Math.pow(1 / (1 + suposto_juros), per_int));
  ...
  if (opcao == 3) {
    parcela_float =
      (montante_float * juros_float) /
      (1 - Math.pow(1 / (1 + juros_float), per_int));
  ...
  if (opcao == 4) {
    montante_float =
      (parcela_float * (1 - Math.pow(1 / (1 + juros_float), per_int))) /
      juros_float;
